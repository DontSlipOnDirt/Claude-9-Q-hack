...
VOICE_AGENT_URL = "http://127.0.0.1:5003/meal-suggestions"
VOICE_AGENT_TIMEOUT_SECONDS = 90
...
CONVERSATIONS_DIR = VOICE_DIR / "conversations"
CONVERSATIONS_DIR.mkdir(parents=True, exist_ok=True)
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
LOG_FILE = CONVERSATIONS_DIR / f"log_{timestamp}.txt"
...
# def query_llm_agent(user_input):
def query_voice_agent(user_input):
    """Call voice agent to get meal suggestions."""
    response = requests.post(
        VOICE_AGENT_URL,
...
def main_loop():
    ...
    while True:
        ...
        try:
            agent_response = query_voice_agent(user_text)
        except requests.RequestException as exc:
            log_interaction("System", f"Voice agent request failed: {exc}")
            agent_response = "I could not reach the voice agent backend. Please check if it is running."
