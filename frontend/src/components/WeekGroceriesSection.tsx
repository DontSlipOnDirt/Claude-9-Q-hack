...
function BasketLineThumb({ image }: { image: string }) {
  ...
  return <div className="w-8 h-8 bg-muted rounded-md flex items-center justify-center text-sm shrink-0">{image}</div>;
}
      ...
      <div className="rounded-2xl border border-border bg-card/80 shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-border bg-muted/30 flex flex-wrap items-start justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-foreground text-base leading-tight">Week groceries</h3>
        ...
        <div className="px-4 py-4">
          ...
          {grouped.length === 0 ? (
            <p className="text-sm text-muted-foreground italic py-2">No groceries yet for this week.</p>
          ) : (
                  ...
                  <ul className="space-y-0 divide-y divide-border rounded-lg border border-border overflow-hidden bg-background/50">
                    {items.map((item) => (
                      <li key={item.id} className="flex items-center gap-2.5 px-3 py-2.5">
                        ...
                        <span
                          className={cn(
                            "text-[10px] font-bold tabular-nums px-2 py-0.5 rounded-full bg-secondary text-secondary-foreground shrink-0",
                          )}
                        >
                          ×{item.quantity}
                        </span>
                        <span className="text-sm font-semibold text-foreground tabular-nums w-[4.25rem] text-right shrink-0">
